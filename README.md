...
## 📸 نظام Plate Recognizer Snapshot API (جديد! ⭐⭐⭐⭐)

**تكامل شامل مع Plate Recognizer Snapshot API لالتقاط وتحليل صور المركبات**

### 🎯 الميزات الرئيسية:
- ✅ **التعرف على اللوحات** - كشف تلقائي لأرقام اللوحات مع OCR دقيق
- ✅ **تحليل المركبات** - استخراج نوع السيارة، الموديل، اللون، والمزيد
- ✅ **تخزين مرن** - اختر بين S3 (افتراضي) أو قاعدة البيانات
- ✅ **تجزئة SHA256** - للتحقق من سلامة الصور
- ✅ **عتبة الثقة** - تصفية النتائج منخفضة الجودة
- ✅ **معالجة دفعية** - معالجة صور متعددة مع شريط تقدم
- ✅ **Docker جاهز** - دعم كامل لـ Docker و docker-compose

### 📦 أوضاع التخزين:

#### 1. تخزين S3 (افتراضي - موصى به للإنتاج):
```bash
STORE_IMAGES=s3
# الصور تُرفع إلى AWS S3 أو MinIO
# البيانات الوصفية فقط تُخزن في قاعدة البيانات
```

#### 2. تخزين قاعدة البيانات (اختياري - للتطوير):
```bash
STORE_IMAGES=db
# الصور تُخزن مباشرة في PostgreSQL كـ bytea
# مناسب للاختبار أو النشر الصغير
```
...
# 2. تعبئة بيانات الاعتماد في .env
# PLATE_API_KEY=your_key
# DATABASE_URL=postgresql://...
# AWS_ACCESS_KEY_ID=... (إذا STORE_IMAGES=s3)

# 3. إنشاء قاعدة البيانات
psql -U postgres -d platenet -f db_schema.sql

# 4. تحضير قائمة الصور
echo "https://example.com/car1.jpg" > images.txt
echo "/path/to/car2.jpg" >> images.txt
...
### 📖 التوثيق الكامل:
📘 **[PLATE_RECOGNIZER_SNAPSHOT_README.md](PLATE_RECOGNIZER_SNAPSHOT_README.md)** - دليل شامل ثنائي اللغة مع:
...
### ⚠️ تحذيرات الخصوصية:
- بيانات اللوحات معلومات شخصية في العديد من الولايات القضائية
- يجب الحصول على موافقة مناسبة قبل التقاط البيانات
- الالتزام بـ GDPR، CCPA والقوانين المحلية
- تنفيذ سياسات الاحتفاظ بالبيانات وضوابط الوصول
